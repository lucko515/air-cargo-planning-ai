...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
        ...
        def load_actions():
            ...
            """
            loads = []
            #The definition of the Action is : Load(cargo, plane, airport)
            #so we iterate through cargos then planes and airpots
            #for each of these we make Load actions 
            for cargo in self.cargos:
                for plane in self.planes:
                    for port in self.airports:

                        #Our preconditions are this: At(c, a) ∧ At(p, a) ∧ Cargo(c) ∧ Plane(p) ∧ Airport(a)
                        #But we will need to check just for At(c, a) and (^) At(p, a)
                        precond_pos = [expr("At({}, {})".format(cargo, port)),
                                       expr("At({}, {})".format(plane, port))]
                        precond_neg = []

                        #After performing some action (in this case Load) the effect to our problem is that some
                        #states should be removied (effect_rem) and some states are added
                        #EFFECT: ¬ At(c, a) ∧ In(c, p))
                        effect_add = [expr("In({}, {})".format(cargo, plane))]
                        effect_rem = [expr("At({}, {})".format(cargo, port))]

                        #This is where Load action is defined with all preconditions and effect to be
        ...
        def unload_actions():
            ...
            """
            unloads = []
            #The definition of the Action is : Unload(cargo, plane, airport)
            #so we iterate through cargos then planes and airpots
            #for each of these we make Load actions
            for cargo in self.cargos:
                for plane in self.planes:
                    for port in self.airports:
                        ...
                        #We are just checking for In(c, p) and At(p, a)
                        ...
                        #This is where Unload action is defined with all preconditions and effect to be
    ...
    def result(self, state: str, action: Action):
        ...
        for fluent in old_state.pos:
            #if that fluent is not in removal_effect from acton
            #we are going to add it to positve fluents for new state
            if fluent not in action.effect_rem:
                new_state.pos.append(fluent)

        #going through fluents that are added by the action
        for fluent in action.effect_add:
            #if that fluent is not already in positive fluents of new state 
            #we are going to add it
            if fluent not in new_state.pos:
                new_state.pos.append(fluent)
        #going through negative fluents from old state
        for fluent in old_state.neg:
            #if that negative fluent is not in adding fluents of the action
            #we are going to add it to negative flents for new state
            if fluent not in action.effect_add:
                new_state.neg.append(fluent)
        #going through fluents that are being removed by the action
        for fluent in action.effect_rem:
            #if that fluent is not already in negatives for the new state
            #we are going to add it
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        #Help for understanding this heuristic I have found in this Forum post:
        #https://discussions.udacity.com/t/understanding-ignore-precondition-heuristic/225906
        

        #accessing the logical expressions
        kb = PropKB()
        ...
        #if we had found our goals count var will stay at 0
        count = 0
        
        #going through all goals that we have in our problem
        for goal in self.goal:
            #if that goal is not alredy in positive states -
            #which means that we haven't reach the goal yet
            #we are increasing counter by one
